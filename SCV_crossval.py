...
class SupervisedCaviarVision(Dataset):
    ...
    def preprocess_image(self, image, desired_image_size):
        ...
        return torch.Tensor(resized).to(torch.uint8)
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, test_idx) in enumerate(k_fold.split(np.arange(len(dataset)))):
        ...
        for epoch in range(num_epochs):
            ...
            #         outputs_int.append(torch.argmax(output).int())

            #     for output in outputs:
            #         outputs_float.append(torch.argmax(output).float())

            #     [
            #         metric.update(torch.Tensor(outputs_int), labels)
            #         for metric in train_metrics_int
            #     ]

            #     [
            #         metric.update(torch.Tensor(outputs_float), labels)
            #         for metric in train_metrics_float
            #     ]

            #     # report performance at the end of every epoch
            #     if (batch_idx + 1) == len(train_dl):
            #         print(
            #             "int metrics \t acc: {:.4f}, pre: {:.4f}, rec: {:.4f}, f1: {:.4f}".format(
            #                 *[metric.compute() for metric in train_metrics_int],
            #             )
            #         )
            #         print(
            #             "float metrics \t acc: {:.4f}, pre: {:.4f}, rec: {:.4f}, f1: {:.4f}".format(
            #                 *[metric.compute() for metric in train_metrics_float],
            #             )
            #         )
            # # reset all metrics before the next epoch
            # [metric.reset() for metric in train_metrics_int]
            # [metric.reset() for metric in train_metrics_float]
            ...
            with torch.no_grad():
                ...
                for test_inputs, test_labels in test_dl:
                    ...
                    # need to debug this fucking shit

                    for output in test_outputs:
                        test_outputs_int.append(torch.argmax(output).int())
